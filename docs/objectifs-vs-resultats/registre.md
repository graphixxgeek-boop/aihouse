...
| moise-tables-de-loi | 2026-09-23 | 2026-10-23 | 1 | trouvaille réellement suivie d'un geste sur la charte | `docs/moise-tables-de-loi/index.md` (colonne « Suite donnée ») | Le chiffre est volontairement BAS et volontairement exigeant. Bas, parce qu'un outil qui touche un document classé MAITRE ne doit pas être poussé au rendement : son mérite n'est pas de trouver beaucoup, c'est d'empêcher une décision prise sur une mesure fausse. Exigeant, parce que l'unité n'est pas « une trouvaille » mais « une trouvaille SUIVIE D'UN GESTE » — c'est le seul compteur qui ne peut pas se remplir tout seul. Repère mesuré à sa création : son premier passage a produit 3 constats et 2 gestes immédiats (table régénérée, mémoire amorcée). |
| abraham-les-references | 2026-09-23 | 2026-10-23 | 2 | document distinct réellement analysé, hors la charte | `docs/abraham-les-references/index.md` | Le chiffre mesure la GÉNÉRICITÉ, pas l'activité : un analyseur générique qui ne sert qu'un seul document est un analyseur spécifique déguisé, et c'est exactement l'erreur qui l'a fait naître. Deux documents distincts, c'est le minimum qui prouve que la généricité n'est pas décorative. Repère à sa création : trois documents déjà analysés le jour même (la charte via Moïse, les règles de travail, la philosophie). |
| agent-du-temps | 2026-09-24 | 2026-10-24 | 3 | mesure estimation/réel historisée | `docs/agent-du-temps/estimations.md` | Le chiffre est le SEUIL D'AJUSTEMENT lui-même, pas une cible d'activité : en dessous de trois mesures, `ajusterDepuisHistorique()` refuse de proposer un facteur, parce qu'un facteur calculé sur deux points ressemble pourtant à une statistique. L'objectif est donc atteint exactement le jour où l'outil devient capable de faire ce pour quoi il existe — ajuster les estimations suivantes. Mesurer ses SOLLICITATIONS n'aurait rien dit : on peut lui demander l'heure vingt fois sans jamais rien apprendre. |
