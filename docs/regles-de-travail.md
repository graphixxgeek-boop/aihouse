...
### 2bis. La FORME d'une question — rapatriée de la charte le 2026-09-23 (tâche #632)

*(Ces trois blocs vivaient dans l'Article 16 de `CLAUDE.md`, rechargé à chaque message, alors que
la §2 ci-dessus déclare depuis toujours accueillir « seuls les points spécifiques à la méthode de
questionnement elle-même ». C'était leur domicile, ils étaient juste ailleurs. **Déplacés mot pour
mot, pas une syllabe reformulée** — paraphraser en déplaçant suffit à perdre la raison qui a fait
naître la règle, et ce projet l'a déjà payé deux fois (leçon L20). La charte garde l'OBLIGATION
de poser les questions ; ce document dit COMMENT elles se posent.)*
...
1. Relancer un serveur de développement à jour (redémarré si besoin pour garantir que c'est bien
   ...
   **La phase 2 doit intercaler de vrais tours autonomes entre les messages humains, jamais
   ...
   visiteur n'a pas ce trou : `app/page.tsx` relance un tour autonome toutes les 21 secondes, en
   parallèle de ce qu'il tape. Précision vérifiée le lendemain, parce que la première rédaction en
   faisait un absolu : c'est le comportement PAR DÉFAUT (`automatic` vaut `true` au démarrage et
   après un reset), mais il s'interrompt si le visiteur met en pause, s'il reprend une partie
   sauvegardée, ou après une erreur réseau. Une simulation qui n'intercale aucun tour autonome ne
   teste donc pas le parcours normal d'un visiteur.
...
| CHARTER-SPY *(DISSOUS le 2026-09-23 — ligne gardée pour que son nom reste atteignable, jamais pour désigner un outil vivant)* | — | — | **N'existe plus comme entité séparée.** Ses sept fonctions (découpage en Articles, citations croisées, sensibilité, importance, redondances, table de classification) ont migré dans MOÏSE-TABLES-DE-LOI le 2026-09-23, sur décision explicite de l'utilisateur : ce qui ne sert QU'à la charte appartient à l'agent de ce périmètre, pas à l'outil générique qui l'hébergeait. Le surnom reste écrit ici parce qu'il est cité dans une trentaine de fichiers et dans la mémoire du projet : un nom propre sans définition atteignable est une dette de reprise (Article 27), et le retirer en aurait créé une. **Ce qu'il faut savoir en le rencontrant : lire `docs/referentiel/moise-tables-de-loi.md`.** | — | — |
...
| god-of-all-process (`scripts/god-of-all-process.mjs`) | Membre certifié (classique) | 🎖️ | **le tool-brain des PROCESS** (2026-09-22, demande explicite de l'utilisateur) : à la question « qu'est-ce qui gouverne ce que je m'apprête à faire ? », répond quel process s'applique, où il est écrit, qui le surveille, et quelles étapes ont laissé leur trace. Suit l'avancement en le DÉDUISANT des traces réelles sur le disque, jamais via un compteur qui se désynchroniserait ; distingue systématiquement trois états — étape tracée / non tracée / **sans trace vérifiable** — et une SONDE CASSÉE n'est jamais lue comme une étape manquante (l'erreur qu'il a lui-même commise sur trois de ses propres sondes à son premier lancement, corrigée le jour même). Garde-fous Article 24 : chaque process déclaré a-t-il un gardien réel et un document réel, chaque tension déclarée cite-t-elle des process encore existants, et l'identité de session (version de Claude) est-elle déposée — sans quoi chaque rapport produit porte « Version de Claude : non renseignée » | gratuit | à la demande (`node scripts/god-of-all-process.mjs "<tâche>"` pour le réflexe, sans argument pour l'état du dispositif) |
| process.simulation.guardian (`scripts/process-simulation-guardian.mjs`) | Agent | 🎖️ | **référent de discipline d'exécution du protocole de simulation** (2026-09-22, demande explicite de l'utilisateur, sur le modèle de circle-process-guardian avec quatre différences voulues). Se consulte AVANT, pas seulement après : une simulation coûte une heure de vrai quota, et les deux défauts de full_sim18 étaient visibles dans le scénario avant le lancement. Porte cinq LEÇONS, chacune payée par une simulation entière (tours autonomes en phase 2, pseudo envoyé avant d'écrire, port réel du serveur, verrous qui ne figent pas la boucle, dossier cherché sur tout l'historique) et vérifie les 9 moments narratifs exigés. BLOQUE par défaut un lancement qui échoue, avec passage en force possible à condition d'une raison écrite archivée avec la simulation. Un champ non renseigné est nommé comme tel, jamais lu comme un « non » (bug réel de sa première version, trouvé par son propre test). Contrôle d'après : réutilise `checkPhase2Autonomy()`/`checkObserverIdentified()` de summarize-simulation-log.mjs, jamais un second calcul, et vérifie les 5 étapes d'archivage — a trouvé dès son premier lancement réel que la ligne KPI de full_sim18 manquait | gratuit | à la demande (`brief` avant de lancer, `postflight <journal> <nomSimu>` après), et à chaque simulation Article 18 |
| angel-of-ia-process (`scripts/angel-of-ia-process.mjs`) | Agent | 🎖️ | **gardien de la CONDUITE, jamais du déroulé** (2026-09-22, nom donné par l'utilisateur) : god-of-all-process surveille le déroulé d'ACTIVITÉS, angel surveille le respect des règles de travail par ceux qui travaillent — agent ET utilisateur, notés pareil (« que ce soit pour moi ou pour toi »). Son apport propre, que rien d'autre ne fait : **le croisement des horodatages** — `.tool-usage-history.json` contre `git log`, pour savoir si les outils à consulter AVANT d'agir (tool-brain, Smart Conso API, SMART-CONSO-TOKEN) l'ont vraiment été avant. Trois états jamais deux : respecté / manqué / **non concluant** — seule une origine que l'agent a déclarée sciemment (`spontane`, `demande`) peut fonder un manquement nommé ; un simple `cli_direct` dit qu'un outil a été lancé, jamais pourquoi, et devient un signal montré sans imputer (leçon payée par SIX fausses accusations à son premier passage, puis six encore au deuxième). Les 6 règles qui ne se jouent que dans la conversation sont DEMANDÉES, jamais devinées, et le verdict refuse d'être vert tant qu'elles restent sans réponse | gratuit | à la demande (`node scripts/angel-of-ia-process.mjs`), et relayé par god-of-all-process à chaque Ronde |
| data-archangel (`scripts/data-archangel.mjs`) | Agent | 🎖️ | **veille sur la CIRCULATION des données dans l'Agence** (2026-09-22, nom donné par l'utilisateur) : 53 sources inventoriées dynamiquement (journaux locaux, registres, série KPI — jamais une liste tenue à la main), et pour chacune QUI la relit vraiment. Prend le problème par les DEUX bouts dans un seul verdict (calibrage de l'utilisateur) : la donnée écrite que personne ne relit, et l'outil qui pourrait gagner à lire ce que ses voisins de terrain consultent déjà — cette seconde suggestion n'est jamais une table d'affinités écrite à la main mais une ressemblance DÉDUITE des lectures réelles. Troisième rôle, explicitement demandé : servir l'AGENT en priorité (`briefing [filtre]` = tout ce que l'équipe sait sur un sujet et où ça vit, avant un gros travail). Distingue déclarer de lire — son premier passage annonçait « 100 % branché » parce que le catalogue du paysage cite toutes les sources sans en consommer aucune ; 74 % après correction, 13 registres écrits que rien ne relit | gratuit | à la demande (`node scripts/data-archangel.mjs`, ou `briefing <sujet>`), et à chaque Ronde |
| tool-brain (`scripts/tool-brain.mjs`) | Membre certifié (classique) | 🎖️ | cerveau élargi des rappels d'outils : généralise find-brain à TOUT le catalogue PRESTATIONS de LE-COORDINATEUR (description de tâche et/ou fichier ciblé) ; rappel post-commit centralisé (find-booster + find-deep-booster + menu PRESTATIONS en un seul bloc) ; rapport KPI (outils jamais sollicités, auto-diagnostic borné à son propre périmètre) ; **le seul signal POSITIF du paysage** depuis le 2026-09-25 (tâche #822, sa demande : « les utilisations spontanées des outils [...] sont à flagger comme "signe trés positif" ») — l'initiative est DÉRIVÉE (un appel en ligne de commande qui n'est ni un item de Ronde ni lancé par un crochet), jamais auto-déclarée, et l'outil REFUSE de conclure sans la source des crochets parce qu'un faux OR félicite pour un mérite inexistant ; il imprime juste en dessous la cause racine (`findOriginesJamaisEcrites()`, tâche #823) : quelle origine du vocabulaire aucun chemin automatique ne produit et que le journal n'a jamais portée — une seule à ce jour, « spontane », que `diagnoseToolBrainSelf()` LISAIT pour en tirer un reproche permanent rendu sur du vide, et dont `ORIGINES_DE_JUGEMENT` d'angel-of-ia-process dépend encore pour moitié | gratuit | à la demande (`node scripts/tool-brain.mjs "<tâche>" [--file <chemin>]` / `rapport`), automatiquement à chaque commit (rappel), et à chaque Ronde CIRCLE-TASKS (rapport) |
...
| SAFE-EXPORT (`scripts/safe-export.mjs`) | Agent | 🎖️ | **SEPTIÈME Gardien sacré, par sa COUCHE LÉGÈRE seulement** (2026-09-22) — répond à deux questions distinctes : l'Agence est-elle exportable vers un autre projet ? le code est-il reprenable par une autre IA ? Quatre détecteurs : fuites de spécificité dans un fichier déclaré générique, termes employés sans définition atteignable, mécanismes sans raison écrite, dépendances à un outillage particulier. Détecte aussi une absence de blueprint ou un blueprint mal construit. Porte la MOITIÉ 1 de l'évolutivité (pouvoir partir) — TOOL-LEARNING porte la moitié 2 | gratuit (couche légère) ; le scan profond est payant et exceptionnel | à CHAQUE commit via le crochet post-commit, jamais un item de Ronde — même régime que les six autres Gardiens |
| TOOL-LEARNING (`scripts/tool-learning.mjs`) | Agent | 🎖️ | vérifie que les outils APPRENNENT — et que l'agent les aide vraiment à progresser (2026-09-22). Frontière avec CASSANDRA-RH, tranchée par l'utilisateur : elle juge l'ÉTAT et les MOYENS (a-t-il mémoire, rapports, évolutivité ?), lui la TRAJECTOIRE et l'USAGE (s'en sert-il ? est-il meilleur qu'avant ?) — d'où le verdict « archive seulement », qu'elle ne peut structurellement pas rendre. Quatre preuves de natures SÉPARÉES : trois mesurent ce que l'outil fait seul, une mesure l'apport de l'agent (un outil beaucoup corrigé est *enseigné*, pas apprenant). Porte la MOITIÉ 2 de l'évolutivité (devenir meilleur) | gratuit — relit des registres déjà produits | item de Ronde (`tool-learning`) : il juge une TRAJECTOIRE, donc trois passages minimum avant de conclure — jamais à chaque commit |
| MOÏSE-TABLES-DE-LOI (`scripts/moise-tables-de-loi.mjs`) | Agent | 🎖️ | l'agent dédié au SEUL périmètre de la charte (2026-09-23, tâche #613, demande explicite : « il est dédié à ce qui est UNIQUEMENT DÉDIÉ à Claude.md : pas de chevauchement »). Sa règle d'appartenance est un test unique : une capacité qui ne sert QU'à la charte lui appartient ; une capacité qui vaut pour n'importe quel document reste chez l'outil générique et il l'APPELLE. C'est à ce titre que les sept fonctions de CHARTER-SPY ont MIGRÉ de `smart-conso-token.mjs` vers lui, déplacées telles quelles, tests inchangés. **Ce qu'il apporte et que rien ne faisait** : le PORTEUR de chaque règle en trois états — porté (nomme un mécanisme qui existe) / sans porteur (sa prose EST le mécanisme, Article 27) / FANTÔME (nomme un mécanisme introuvable, pire qu'une absence puisque ça rassure à tort) ; la NATURE de chaque règle et le geste qu'elle commande (LOI / MODE D'EMPLOI D'OUTIL / DISCIPLINE SANS PORTEUR / INVENTAIRE), proposée par la mesure et tranchée par l'utilisateur, sa décision survivant à la régénération ; la vérification qu'un document d'accueil existe ET porte déjà le contenu avant tout renvoi ; deux garde-fous de fraîcheur gratuits à chaque commit ; et une MÉMOIRE au grain de la règle touchée, qui répond à « comment on a fait la dernière fois ? » et met en évidence les gestes déjà ANNULÉS. Il porte aussi le process « analyse et plan d'action de la charte » (`docs/analyse-charte-process-detail.md`, 10 étapes, dont 6 déclarées non mesurables plutôt que comptées au vert). **Ce qu'il ne fait jamais** : modifier la charte (classée MAITRE par tool-brain), écrire l'analyse ou le plan d'action à ma place, trancher une nature, bloquer quoi que ce soit | gratuit — relit la charte et le dépôt local, et appelle les outils existants plutôt que de recalculer | HORS Ronde, exclusion motivée dans `CIRCLE_AUTO_COVERED_REGISTRIES` : sa partie gratuite tourne déjà à chaque commit, et un item de plus contredirait la tâche #612 qui cherche à RÉDUIRE le nombre de rapports |
| Abraham-les-references (`scripts/abraham-les-references.mjs`) | Agent | 🎖️ | **l'outil MAÎTRE des documents à règles numérotées** (2026-09-23, tâche #619, décision explicite de l'utilisateur : « on separe et l'outil Maitre s'appelle : Abraham-les-references »). Il fait sur N'IMPORTE quel document à unités numérotées ce que MOÏSE-TABLES-DE-LOI ne fait que sur la charte : détecter la FORME des unités (quatre formes connues, `detecterForme()` — `**Article N — Titre.**`, `### N.N`, `## N.`, `### N.`) plutôt que de la supposer, mesurer chaque unité (poids, citations dans le dépôt, porteur déclaré en TROIS états, nature proposée), juger la PERTINENCE (rendement faible = beaucoup de texte pour peu de citations, seuil DÉRIVÉ du document lui-même et jamais recopié — Article 24), trouver les redondances et les recouvrements non déclarés, vérifier qu'un document d'accueil existe ET porte déjà le contenu avant tout renvoi, et tenir la mémoire des opérations. **Pourquoi il est séparé de MOÏSE** : ce qui vaut pour tout document appartient au générique, ce qui ne sert QU'à la charte appartient à l'agent de ce périmètre — MOÏSE l'APPELLE et n'a gardé que ses adaptateurs de vocabulaire (« Article » au lieu de « règle »). Le refactor a rendu les 8 blocs dupliqués que CLONE-HUNTER avait signalés (25 → 17). Prouvé sur trois documents sans une ligne de configuration : `CLAUDE.md` (via MOÏSE), `docs/regles-de-travail.md`, `docs/philosophie-et-politique.md` | gratuit — relit le dépôt local, aucun appel API | HORS Ronde, même motif que MOÏSE : ce qu'il sait dire gratuitement remonte déjà par l'agent qui l'appelle, et un item de plus contredirait la tâche #612 |
| AGENT DES NOMS (`scripts/agent-des-noms.mjs`) | Agent | 🎖️ | **sa fonction principale est de minimiser le risque technique d'un renommage en masse** (cadrage corrigé par l'utilisateur le 2026-09-24 : « cet agent a pour fonction principale de minimiser le risque technique lié au changement de noms en masse »). Il inventorie sur le vrai dépôt toutes les occurrences d'un nom et les trie en cinq NATURES — import de code, commande écrite, chemin de registre, mention vivante, et **trace historique, qui ne se touche jamais**. **La règle centrale tombe de la mesure, pas d'une idée** : renommer `cassandra-rh` touche 2 095 occurrences, dont NEUF imports de code (qu'un test cassé attrape en dix secondes) et 496 lignes de `docs/suivi/`, c'est-à-dire de l'HISTOIRE — **un renommage déplace le code vivant et laisse l'histoire intacte**, et ce qui relie les deux est un ALIAS, jamais une réécriture. Signale aussi les noms qui CONTIENNENT le nom visé (`memento` emporterait `memento-weight`), piège qu'un remplacement de texte aveugle ne voit jamais. **Il ne renomme rien, délibérément** : il planifie avant, il vérifie après. Sa seconde moitié tient la GOUVERNANCE du nommage — l'utilisateur choisit tous les noms, `enregistrerBapteme()` refuse un nom sans `parUtilisateur: true`, et une marque `NOM PROVISOIRE` datée est cherchée à chaque passage. | gratuit — un `git grep` et des motifs, jamais un parseur | deux commandes : `renommage <ancien> <nouveau>` avant le geste, `verifier <ancien>` après · item de Ronde (`agent-des-noms`) pour la seule moitié gouvernance, la moitié renommage répondant à un événement |
| AGENT-DU-TEMPS (`scripts/agent-du-temps.mjs`) | Agent | 🎖️ | rend l'heure et la date au référentiel France **en déclarant toujours leur SOURCE** — « réseau » si une API de temps a répondu, « système » si c'est l'horloge locale, « aucune » et alors il refuse de répondre. Donne aussi l'horodatage UTC exact qu'attend `docs/suivi/`. Héberge la mémoire des estimations (durée, tokens, appels API) et leur comparaison au réel, partagée par la Ronde et par la simulation (2026-09-24, demande explicite). **Son défaut fondateur est mesuré** : une IA ne sait pas l'heure, elle la déduit du dernier horodatage vu passer — la nuit de sa construction, une ligne de suivi a été datée de quinze minutes dans le futur et un garde-fou a refusé le commit. **État réel dans cet environnement** : les deux API de temps essayées rendent HTTP 403, refusées par la politique réseau — la source est donc « système », et l'outil le dit à chaque passage. | gratuit — une requête HTTP de temps, et l'horloge système en repli | item de Ronde (`agent-du-temps`) : demande explicite (« teste l'agent à la Ronde »). Le seul item dont le vrai résultat est une PHRASE à lire, jamais un compte — un agent du temps dont personne ne vérifie la source finirait par rendre l'horloge locale en silence. |
| THE-EQUALIZER (`scripts/the-equalizer.mjs`) | Agent | 🎖️ | rend UN verdict par domaine (l'Agence, les documents, le code, le jeu) contre les exigences écrites de `docs/referentiel/standards.md`, et nomme ce que PERSONNE ne vérifie (2026-09-23). Répond à la question de l'utilisateur telle qu'il l'a posée : « qui se charge de vérifier que tout est à niveau [...] TOUT TOUT doit être à niveau ». C'est un RASSEMBLEUR : il n'a aucun détecteur propre, il relit le référentiel et relaie des verdicts déjà calculés — le retard outil par outil vient d'`integrationAudit()` (LE-COORDINATEUR), jamais d'une seconde boucle. Ce qu'il apporte qu'aucun contrôleur ne peut apporter seul : vingt outils qui disent « ma part va bien » ne disent jamais « tout va bien », puisque personne ne détient la liste de ce qui DEVRAIT être vérifié. Trois signaux lui sont propres — l'exigence sans vérificateur, le vérificateur FANTÔME (annoncé, inexistant : pire qu'une absence, il rassure à tort) et le niveau ORPHELIN rattaché à aucun domaine. Frontière avec CASSANDRA-RH et TOOL-LEARNING : elles jugent un outil (son état, sa trajectoire), lui juge la COUVERTURE des exigences elles-mêmes | gratuit — relit un document et des verdicts déjà produits | item de Ronde (`a-niveau`) : question de période, jamais de commit — à chaque commit le même verdict se répéterait jusqu'à ne plus être lu |
| integration-outil (`scripts/integration-outil.mjs`) | Membre | — | dit, registre par registre, ce qui reste à renseigner pour faire entrer un nouvel outil dans l'Agence (2026-09-22). Né d'un constat chiffré : faire entrer SAFE-EXPORT puis TOOL-LEARNING a demandé SEPT inscriptions manuelles chacun, chacune signalée en FAISANT ÉCHOUER un test — jamais avant. Il ne remplace aucun de ces sept garde-fous (ils continuent d'échouer si l'inscription manque) : il inverse le MOMENT, la liste devenant demandable avant de commencer. La liste des registres est LUE dans les fichiers réels, jamais recopiée (Article 24). Son propre garde-fou `findLecteursCasses()` refuse de conclure quand un lecteur ne reconnaît plus la forme de son registre — il a attrapé trois de ses propres lecteurs cassés à sa première exécution, avant tout chiffre faux | gratuit — lecture de fichiers déjà sur le disque | jamais un item de Ronde : il répond à un ÉVÉNEMENT (un outil qui arrive), pas à un calendrier. Son obligation vit dans l'entrée « Intégration d'un nouvel outil » de PROCESSES |
