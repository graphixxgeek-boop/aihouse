...
| CLONE-HUNTER (`scripts/clone-hunter.mjs`) | Agent | 🎖️ | détecte des blocs de lignes identiques (après normalisation d'espaces) répétés dans lib/scripts/app/components (hors components/ui, exclu — kit shadcn/Radix vendu tel quel, duplication assumée par design) — v1 littérale, jamais sémantique ; vérifié live à trouver 7 clusters réels dans scripts/, dont `loadJson()` dupliqué entre smart-conso-api.mjs et smart-conso-token.mjs | gratuit | Membre complet dès la construction (demande explicite de l'utilisateur) — `clone-hunter-run` rejoint CIRCLE-TASKS en lancement réel à chaque passage (thème "Passages réels (smoke run)"), cf. `docs/referentiel/clone-hunter.md` |
