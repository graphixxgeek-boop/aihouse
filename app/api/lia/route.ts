...
export async function POST(request: Request) {
    ...
    try {
        ...
        const negotiationContext=revealed?"Les bonus de la maison (roulette) ne sont obtenus que si l’observateur déclenche lui-même le tirage — ça ne coûte rien de le lui rappeler. RÉFLEXE PAR DÉFAUT, pas simple possibilité : dès que l’observateur te demande quelque chose (une réponse, un service, une action de ta part), ton premier réflexe est de réclamer un tirage en échange, ou de conditionner clairement ce que tu vas faire à ce tirage — pas seulement quand « la situation s’y prête » par hasard. Tu restes libre de refuser purement et simplement si ton personnage le justifie, mais l’absence de contrepartie doit être l’exception que tu choisis, pas l’oubli. Jamais par supplique, jamais un ton de service client : c’est un prix que tu fixes, cash, dans ton propre registre.":undefined;
        ...
        if(revealed&&!life.dossierText&&TRAP_ORDER.every(t=>life.dossierTraps?.[t])&&(life.bonusLog?.length??0)>0&&env.GEMINI_API_KEY){
            ...
            const [liaFragment,noeFragment]=await Promise.all([generateDossierFragment(env.GEMINI_API_KEY,model,"Lia",dossierEvidenceFor(1)),generateDossierFragment(env.GEMINI_API_KEY,model,"Noé",dossierEvidenceFor(2))]);
