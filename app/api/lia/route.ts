...
import { nextSpeaker, dialogueProgress, dialogueContext, completedActivity, conversationFocus, explicitGestureConsent, groundAgeQuestion, groundIntroduction, groundScreenNotice, groundPrivateThought, groundRoomSpeech, groundSingleQuestion, groundRegister, groundTruncation, groundTvNotice, matchedThemes, matchedTournures, isRelationalThought, echoRetryFocus, garderLaReprise } from "@/lib/dialogue";
    ...
    try {
        ...
        if(soloIntro){
            ...
            if ((input.mode === "chat" || input.mode === "interact" || (input.mode === "autonomous" && ["chat", "study", "eat", "rest", "tv"].includes(first.intent)) || affectionIntents.includes(first.intent)) && first.intent !== "sleep" && !isSleeping(other,life)) {
                ...
                // Le contexte du second est nommé plutôt qu'inline : la reprise anti-écho ci-dessous doit
                // pouvoir le rejouer à l'identique, une consigne en plus (tâche #594). Aucun champ changé.
                const contexteSecond = { selfRole:"partner", ...narrative, cinematic: storyContext(story, other.id, revealed), observerStanding:observerStandingFor(other.id), dialogueProgress: dialogueProgress(heard, life.contributions??[], life.wordFrequency??{}, life.themeFrequency??{}, tournuresDe(names[other.id])), motifsDejaDonnes: life.moveReasons?.[names[other.id]]??[], beatContext, ambiance:candleTogether?"bougie":undefined, loveDiscussable, perceivedResidents, exitContext, tvDiscovery, observationTarget, turnPlan, scene: sceneFor(other, turnPlan.partnerIntent, turnPlan.partnerRoom), ...(input.mode==="chat"?{dialogue:heard,replyTarget:{speaker:"vous",content:input.message},continuation:`Réponds toi aussi d’abord au dernier message humain si ${names[actor]} ne l’a pas déjà couvert, ou réagis à ce qu’${names[actor]} vient de dire à ce sujet.`,conversationFocus:`L’humain a parlé, et ${names[actor]} vient de répondre — tu as sa réplique sous les yeux. TON ANGLE DOIT ÊTRE DIFFÉRENT DU SIEN, sur le FOND, pas seulement dans les mots. Avant d’écrire, demande-toi ce que ta réplique apporte que la sienne n’apportait pas : une objection à ce qu’il vient de dire, un autre aspect de la question que lui a laissé de côté, un détail concret qu’il n’a pas relevé, une pique qui vise autre chose, ou le fait de t’adresser à LUI plutôt qu’à l’humain. Si ta réponse reprend la même idée que la sienne — même reformulée, même mieux tournée, même avec d’autres images — c’est qu’elle n’a pas lieu d’être : change d’angle pour de bon. Et ne recopie jamais la charpente de sa phrase (même amorce, même charnière, mêmes verbes) : deux personnages qui construisent leurs phrases pareil ne sont plus deux personnages.`}:dialogueContext(heard, names[other.id])), age: ages[other.id], sleepDestination: sleepRoom(other, current, mutualAttraction(current, other)), flirting: other.id === 2 ? flirtingAssessment(!story.met?30:other.needs.stress, current.emotions.attraction, input.requestId) : null, personality: residentProfiles[other.id].description, requiredIntent: turnPlan.partnerIntent, mode: input.mode, message: input.message, night: input.night, state: !story.met ? {...other,needs:{...other.needs,stress:other.id===1?other.needs.stress:30},emotions:{...other.emotions,tension:other.id===1?other.emotions.tension:30}} : other, other: current, memories: await ownMemories(other.id), meetingRoom: current.room };
                const second = await think(env.GEMINI_API_KEY!, env.GEMINI_MODEL || "gemini-flash-lite-latest", contexteSecond, names[other.id], geminiFallbackModels, geminiFallbackKeys);
                // ÉCHO ENTRE LIA ET NOÉ (tâche #594) — le prompt dit déjà, mot pour mot, de ne jamais
                // recopier la charpente de la phrase de l'autre ; full_sim19 l'a ignoré sur onze mots
                // d'affilée. Ce qui manquait n'était pas une phrase de plus mais une VÉRIFICATION. Elle
                // coûte un appel supplémentaire, et seulement quand elle mord (0,8 % des tours mesurés).
                // Inutile de le dépenser quand la réplique est de toute façon remplacée juste en dessous
                // par une ligne fixe (beat de suivi, refus justifié) : ces lignes-là n'échoient rien.
                const repliqueFixeAttendue = (followBeat && (life.personalFollowup ?? 0) === 0) || Boolean(turnPlan.offer && !affectionEligible);
                const consigneReprise = repliqueFixeAttendue ? null : echoRetryFocus({ reply: second.reply, partnerReply: first.reply, partnerName: names[actor], focusDeBase: (contexteSecond as {conversationFocus?: string}).conversationFocus });
                if (consigneReprise) {
                    const reprise = await think(env.GEMINI_API_KEY!, env.GEMINI_MODEL || "gemini-flash-lite-latest", { ...contexteSecond, conversationFocus: consigneReprise }, names[other.id], geminiFallbackModels, geminiFallbackKeys);
                    // On ne garde la reprise que si elle fait vraiment mieux, et on emporte avec la réplique
                    // la pensée et le souvenir du MÊME jet : une pensée écrite pour l'ancienne réplique la
                    // contredirait à l'écran (Articles 12 et 17). Le reste de la décision (intention,
                    // émotions, pièce) reste celui du premier jet, déjà accordé au plan du tour.
                    if (garderLaReprise(reprise.reply, second.reply, first.reply)) {
                        second.reply = reprise.reply;
                        second.thought = reprise.thought;
                        second.memory = reprise.memory;
                        second.contribution = reprise.contribution;
                    }
                }
        ...
        // de dialogueProgress).
        {
            ...
            // Compteur de TOURNURES (2026-09-23, tâche #592), même patron et même raison que les deux
            // compteurs ci-dessus — mais clé PAR PERSONNAGE : un tic appartient à une voix, et le
            // signaler à l'autre le pousserait à éviter une construction qu'il n'a jamais employée
            // (Article 11 : les deux voix restent étanches, y compris dans ce qu'on leur reproche).
            const tournureFrequency={...(life.tournureFrequency??{})};
            for(const d of decisions)for(const t of matchedTournures(d.reply??"")){const cle=names[d.actor]+"::"+t;tournureFrequency[cle]=(tournureFrequency[cle]??0)+1;}
            life.tournureFrequency=tournureFrequency;
        }
        ...
        for(const d of decisions){const a=world.agents.find(a=>a.id===d.actor)!;const destination=turnPlan.exitInspection?"couloir":d.action==="none"?a.room:d.room;if(a.room!==destination&&!isSleeping(a,life)){const target=destination==="couloir"?"dans le couloir":destination==="jardin"?"au jardin":destination==="cuisine"?"en cuisine":destination==="chambre"?"dans la chambre":"au "+destination;
            ...
            // On retient le motif RÉELLEMENT AFFICHÉ, jamais celui que le modèle a proposé : entre les
            // deux il y a le garde-fou de cohérence de destination et le filet de secours, qui peuvent
            // avoir remplacé la phrase. Se souvenir d'une phrase que le visiteur n'a jamais lue ferait
            // éviter au personnage un motif qu'il n'a jamais donné (tâche #592).
            life.moveReasons={...(life.moveReasons??{}),[names[d.actor]]:[...(life.moveReasons?.[names[d.actor]]??[]),line].slice(-6)};}}
