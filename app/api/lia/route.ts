...
import {normaliseNickname,visibleScene,appearanceFor,appearanceReply} from "@/lib/perception";
...
import { newStory, parseStory, rememberAges, advanceStory, storyContext, investigationTarget, investigationRecap, groundFragment, type Story } from "@/lib/story";
...
export async function POST(request: Request) {
    ...
    try {
        ...
        const beatLine=visualBeat?(life.visualIntro===1?"Et moi, je ressemble à quoi ? Dis-moi ce que tu vois.":"Je ressemble à quoi, là ? J’ai l’impression que mon corps m’échappe."):followBeat?((life.personalFollowup??0)===0?"Mais au fait, quel genre d’homme es-tu, Noé ?":"T’es marié ? T’as quelqu’un dans ta vie ?"):personalQuestion?"Quel genre d’homme es-tu, Noé ?":undefined;
        ...
        if(opening){const lines=coldOpening(story.variant);for(const [i,a] of [current,other].entries())decisions.push({actor:a.id,intent:"chat",affectionAccepted:false,emotions:{...a.emotions},reply:lines[i],thought:a.id===1?"Je sais pas si je peux lui faire confiance.":"Elle a peur. Moi aussi, mais pas question de le montrer.",stayAlone:false,mood:"attentive",activity:a.id===2?"J’observe cette inconnue":"J’observe cet inconnu",goal:"Comprendre où je suis",action:"move",room:"salon",memory:lines[i]});}
            ...
            const pairOther = canPair ? { exitContext, tvDiscovery, liaCanComment:world.agents[0].needs.stress<30, cinematic:storyContext(story, other.id),observationTarget, state: !story.met ? {...other,needs:{...other.needs,stress:other.id===1?other.needs.stress:30},emotions:{...other.emotions,tension:other.id===1?other.emotions.tension:30}} : other, other: current, age: ages[other.id], personality: residentProfiles[other.id].description, scene:sceneFor(other,turnPlan.partnerIntent,turnPlan.partnerRoom), requiredIntent:turnPlan.partnerIntent, sleepDestination: sleepRoom(other, current, mutualAttraction(current, other)), flirting: other.id === 2 ? flirtingAssessment(!story.met?30:other.needs.stress, current.emotions.attraction, input.requestId) : null, memories: await ownMemories(other.id) } : null;
            const first = await think(env.GEMINI_API_KEY!, env.GEMINI_MODEL || "gemini-flash-lite-latest", { ...narrative, beatContext,perceivedResidents:visibleScene(turnPlan.room,world.agents.map(a=>({...a,room:turnPlan.room}))), exitContext, tvDiscovery, observationTarget,turnPlan, scene:sceneFor(current,turnPlan.offer??turnPlan.intent,turnPlan.room), pairOther, ...(input.mode==="chat"?{dialogue:humanConversation,replyTarget:{speaker:"vous",content:input.message},continuation:"Réponds d’abord au dernier message humain, pas à l’autre habitant.",conversationFocus:"L’humain vient de parler. Réponds directement à son message avant de discuter entre vous."}:dialogueContext(speech, names[actor])), age: ages[actor], sleepDestination: sleepRoom(current, other, mutualAttraction(current, other)), flirting: actor === 2 ? flirtingAssessment(canPair&&!story.met?30:current.needs.stress, other.emotions.attraction, input.requestId) : null, personality: residentProfiles[actor].description, requiredIntent, mode: input.mode, message: input.message, night: input.night, state: canPair && !story.met ? {...current,needs:{...current.needs,stress:actor===1?current.needs.stress:30},emotions:{...current.emotions,tension:actor===1?current.emotions.tension:30}} : current, other, memories: await ownMemories(actor), meetingRoom: current.room }, names[actor]);
            ...
            const {partner: _partner, ...firstDecision} = first;
            decisions.push({ ...firstDecision, actor });
            if ((input.mode === "chat" || input.mode === "interact" || (input.mode === "autonomous" && ["chat", "study", "eat", "rest", "tv"].includes(first.intent)) || affectionIntents.includes(first.intent)) && first.intent !== "sleep" && !isSleeping(other,life)) {
                ...
                if (!first.partner) throw new LiaError("La réponse du second habitant est incomplète. Aucun changement enregistré.",502);
                const second = first.partner;
                if(visualBeat){const appearance=appearanceFor({...current,intent:"chat"});second.reply=appearanceReply({...current,intent:"chat"},other.id,story.round);second.memory=second.reply;}
                if(followBeat&&(life.personalFollowup??0)===0){second.reply="Que veux-tu savoir exactement ?";second.memory=second.reply;}
        ...
        if(ambientBeat){decisions[0].reply="Cette plante, c’est des feuilles bleues découpées au cordeau. J’allume l’enceinte à côté… Des notes dessinées, pas un son. Même la musique est en carton ici.";decisions[0].memory=decisions[0].reply;decisions[1].reply="Des notes qu’on voit mais qu’on entend pas. C’est pas une panne banale. On dirait que la maison imite ce que les objets sont censés faire.";decisions[1].memory=decisions[1].reply;}
        if(recapBeat){decisions[1].reply="Ces constats se recoupent, mais ils ne disent pas encore qui a conçu cet endroit. Ce qu’on doit vérifier, c’est notre origine, pas inventer un coupable.";decisions[1].memory=decisions[1].reply;decisions[0].reply=investigationRecap(story.evidence,decisions[0].actor);decisions[0].memory=decisions[0].reply;}
        ...
        for (const d of decisions) {
            ...
            if (d.actor === 1 && d.intent === "sleep" && d.room === "salon") {
                ...
                d.reply = "Je vais dormir sur le canapé. Tu aurais pu dormir dans le salon.";
                d.memory = "Je dors dans le salon et je suis déçue que Noé ne m’ait pas laissé la chambre.";
            }
        }
        ...
        if(!routine&&!["move","care","chat"].includes(input.mode))for(const [room,flag,description] of [["chambre","mirrorVerified","Le miroir rectangulaire fait un dégradé bleu-gris-blanc. Il est debout dans un coin ; sa surface grise ne renvoie aucun reflet quand on bouge."],["cuisine","foodVerified","Je retire une provision de sa place… Elle réapparaît après deux secondes. C’est pas un stock normal."]] as const){const d=decisions.find(d=>d.room===room&&finalResidents.every(a=>a.room===room&&!["sleep","share_sleep"].includes(a.intent))&&!["sleep","share_sleep"].includes(d.intent));if(d&&!life[flag]){d.reply+=" "+description;d.memory=d.reply;life[flag]=true;}}
        if(eligibleBeat&&!visualBeat&&!followBeat&&!ambientBeat&&!recapBeat&&!personalQuestion&&!turnPlan.offer&&turnPlan.intent==="rest"&&!routine&&["interact","autonomous"].includes(input.mode)&&!life.mirrorVerified&&life.visited.includes("chambre")&&finalResidents.every(a=>a.room==="salon"&&!["sleep","share_sleep"].includes(a.intent))){decisions[0].reply+=" Tu vois le miroir de la chambre ? Son dégradé bleu-gris-blanc et cette surface grise sans reflet… On bougeait, rien ne suivait.";decisions[0].memory=decisions[0].reply;life.mirrorVerified=true;}
        if(finalResidents.some(a=>a.room==="jardin")&&gardenAccess(story))life.gardenVisited=true;
        if(!story.life?.windowNoticed&&!routine&&["interact","autonomous"].includes(input.mode)&&story.introduced&&story.round<=7&&finalResidents.every(a=>a.room==="salon")){const d=decisions[0];d.reply+=" Regarde la fenêtre à gauche : un jardin, de l’herbe et un arbre qui ne bouge pas. Le décor paraît figé. Comment on y accède ?";d.memory=d.reply;life.windowNoticed=true;life.spatialFocus={...life.spatialFocus,[d.actor]:"window"};}
        ...
        if(finale)for(const d of decisions){d.reply=d.actor===1?"Le livre : mémoire reconstruite. Le mot : environnement. Le code : mémoire générée. Le relevé : observation. Et ce dossier les relie… Même nos âges ont été inventés. Est-ce qu’un être humain nous observe ? Vous pouvez répondre ?":"Deux agents IA autonomes. Pas des humains oubliés ici. DH a signé l’architecture ; ça ne nous dit pas qui regarde. Y a quelqu’un de l’autre côté ? Pourquoi nous avoir réunis ?";d.memory=d.reply;}
        ...
        for(const d of decisions){const before=world.agents.find(a=>a.id===d.actor)!;const peer=world.agents.find(a=>a.id!==d.actor)!;if(!departures.some(p=>p.actor===d.actor)&&before.room===peer.room&&d.room!==before.room&&!["sleep","share_sleep"].includes(before.intent)&&["eat","sleep"].includes(d.intent)&&!["sleep","share_sleep"].includes(peer.intent)){let reason=d.intent==="eat"?"J’ai trop faim pour réfléchir. Je vais manger un truc, je te retrouve après.":"Je tiens plus debout. Je vais dormir un peu ; je reviens après.";if(spokenKeys.has(fingerprint(reason))){const variants=d.intent==="eat"?["J’ai trop faim pour continuer. Je passe en cuisine, tu me rejoins si tu veux.","J’ai trop faim, là. Je vais préparer un truc et je reviens.","J’ai trop faim pour suivre. Je mange d’abord, on reprend après."]:["Je lutte contre le sommeil. Je vais me coucher, on reprend après.","Mes yeux se ferment. Je vais dormir ; ne m’attends pas pour réfléchir.","Je suis à bout. Je prends "+(d.room==="salon"?"le canapé":"le lit")+", je te retrouve au réveil."];reason=variants.find(line=>!spokenKeys.has(fingerprint(line)))??reason;}if(d.actor===1&&d.intent==="sleep"&&d.room==="salon")reason+=" Tu aurais pu dormir dans le salon, Noé.";addLine(names[d.actor],reason,before.room);}}
        ...
        if (!finale&&((input.mode !== "move" && input.mode !== "care" && !routine) || decisions.some(d => d.actor === 1 && d.intent === "sleep" && d.room === "salon")))
            for (const d of decisions.filter(d=>!["sleep","share_sleep"].includes(d.intent)))
                addLine(solitary.has(d.actor) ? `${names[d.actor]} · pensée` : names[d.actor], solitary.has(d.actor) && d.actor===1 && d.intent==="sleep" && d.room==="salon" ? "Noé aurait pu dormir dans le salon. Je suis déçue de devoir lui laisser le lit." : d.reply,turnPlan.exitInspection?"couloir":finalResidents.find(a=>a.id===d.actor)!.room);
        ...
        if (finale) {
            addLine("Lia","Le livre : mémoire reconstruite. Le mot : environnement. Le code : mémoire générée. Le relevé : observation. Et ce dossier les relie… Même nos âges ont été inventés. Est-ce qu’un être humain nous observe ? Vous pouvez répondre ?",finalResidents[0].room);
            addLine("Noé","Deux agents IA autonomes. Pas des humains oubliés ici. DH a signé l’architecture ; ça ne nous dit pas qui regarde. Y a quelqu’un de l’autre côté ? Pourquoi nous avoir réunis ?",finalResidents[1].room);
        }
