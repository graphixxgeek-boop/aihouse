...
export const CIRCLE_ITEMS_CHANGELOG = [
  ...
  },
  {
    ...
    changement: "ajout",
    pourquoi: "L'outil a deux moitiés aux rythmes opposés, et une seule appartient à la Ronde. Sa fonction PRINCIPALE — inventorier le risque technique d'un renommage en masse, corrigée par l'utilisateur le soir même (« cet agent a pour fonction principale de minimiser le risque technique lié au changement de noms en masse ») — répond à un événement : on la sollicite quand un renommage est envisagé, jamais au calendrier. Sa moitié GOUVERNANCE, elle, est exactement périodique, et pour une raison qu'un item de Ronde est le seul à pouvoir porter : un nom posé à titre provisoire ne fait de mal qu'en DURANT, et personne ne se réveille un matin en se demandant si un nom traîne depuis huit jours. Le passage régulier est le mécanisme, pas la bonne volonté. L'item rend deux chiffres qui ne veulent pas dire la même chose et que la Ronde ne doit jamais additionner : les provisoires oubliés sont un retard récent, les noms jamais validés sont la dette de fond (78 sur 78 au jour de sa création) et elle se purge APRÈS la classification de l'iceberg, séquence posée par l'utilisateur lui-même.",
  },
  {
    date: "2026-09-24",
    itemId: "sauvegarde",
    changement: "ajout",
    pourquoi: "Demande explicite de l'utilisateur le soir même, et c'est le SEUL item de toute la Ronde dont la raison ne porte pas sur la qualité du code : elle porte sur ce qu'il perdrait s'il perdait l'accès. Sa formulation : « Je dois sauvegarder le projet de mon côté, c'est une sécurité obligatoire, non ? » — et la réponse est oui, sans redondance avec le dépôt distant, puisqu'une copie chez lui survit à un compte fermé ou à une branche écrasée. Deux livrables aux métiers opposés, et les confondre était l'erreur de départ : le coffre (.zip) est exhaustif et jamais lu, il RESTAURE ; la notice (.txt) est choisie et lue, elle EXPLIQUE, calibrée pour tenir dans la mémoire d'une IA en lui laissant de la place pour travailler. L'historisation ne demande rien de plus, ce qui a levé son inquiétude sur les gigaoctets : le suivi, les simulations et les registres SONT des fichiers du dépôt, donc le coffre du jour contient déjà toute l'histoire du jour, et écraser ne perd rien. Enfin, c'est le seul item dont l'exécution se termine par une LIVRAISON et non par un rapport : une sauvegarde qui reste dans le dépôt qu'elle sauvegarde ne protège de rien, et cette remise est la seule étape qui décide si tout le dispositif sert.",
  },
  {
    date: "2026-09-24",
