...
affection=true;sqlite.exec('DELETE FROM world_requests');{const p=JSON.parse(sqlite.prepare("SELECT content FROM memories WHERE kind='scenario'").get().content);p.life={...p.life,visited:['salon','cuisine','chambre','bureau'],tvSeen:true,ambientSeen:true,ambientVerified:true,recapCount:5,personalAsked:true,visualIntro:2,personalFollowup:3,exitSearched:true,contact:undefined,debrief:undefined};p.pendingDestination={room:'salon',intent:'hug',proposer:2};sqlite.prepare("UPDATE memories SET content=? WHERE kind='scenario'").run(JSON.stringify(p));}sqlite.prepare('UPDATE agent_state SET emotions=?,needs=?').run(warm,JSON.stringify(initialNeeds));response=await post(input('interact',1,{epoch:1}));result=await response.json();assert.equal(result.sharedAffection,'hug');assert.ok(result.agents.every(a=>a.intent==='hug'&&a.room==='salon'));assert.equal(smiley(result.agents[0]),'🥰');
...
assert.equal(residentProfiles[2].sharedBonus,4);assert.equal(residentProfiles[1].sharedBonus,2);
...
const {newStory,rememberAges,advanceStory,storyContext}=await import('../.sites-runtime/test-story.mjs');
...
response=await post(input('interact',1,{epoch:majorEpoch}));assert.equal(response.status,200);result=await response.json();assert.ok(result.agents.every(a=>a.room==='cuisine'));assert.ok(result.story.observations.some(o=>o.includes('réapparaît')));assert.ok(result.messages.slice(-2).every(m=>m.room==='cuisine'));assert.ok(result.agents.every(a=>a.attachment===0));
response=await post(input('interact',1,{epoch:majorEpoch}));result=await response.json();assert.ok(result.agents.every(a=>a.room==='chambre'));assert.ok(result.story.observations.some(o=>o.includes('dégradé gris')));
for(let i=0;i<2;i++){response=await post(input('interact',1,{epoch:majorEpoch}));result=await response.json();assert.ok(result.agents.every(a=>a.room==='salon'&&a.intent==='rest'));}
...
response=await post(input('chat',2,{epoch:majorEpoch,message:'Noé, vous m’entendez ?'}));assert.equal(response.status,423);assert.equal(calls,sleepCalls);
sqlite.exec('UPDATE world_lock SET last_auto=0');response=await post(input('autonomous',2,{epoch:majorEpoch}));assert.equal(response.status,200);assert.equal(calls,sleepCalls);assert.equal((await response.json()).agents[1].intent,'sleep');
...
console.log('Passed: initial kitchen/bedroom discovery, regeneration/mirror, retained room stamps, two salon debriefs, TV, two-pass evidence and debrief, zero-call sleeping silence, selected human priority across speaker rooms, stress from threat and reassurance, attachment bounds.');
...
const corridorPlot={...newStory(),round:8,met:true,introduced:true,sharedMeal:true,life:{...newStory().life,visited:['salon','cuisine','chambre','bureau'],tvSeen:true,ambientSeen:true,ambientVerified:true,recapCount:5,personalAsked:true,visualIntro:2,personalFollowup:3,exitSearched:false}};
...
pp={...pp,evidence:[],life:{...pp.life,ambientSeen:true,ambientVerified:false,recapCount:5}};sqlite.prepare("UPDATE memories SET content=? WHERE kind='scenario'").run(JSON.stringify(pp));const repairCalls=calls;const repairInput=input('interact',2,{epoch:perceptionEpoch});response=await post(repairInput);result=await response.json();assert.equal(response.status,200);assert.equal(calls,repairCalls);assert.equal(result.story.life.ambientVerified,true);assert.ok([
  ...
  'Regarde cette plante : des feuilles bleues, découpées au carré, pas une once de vrai. Et cette enceinte, une fois allumée… des notes qui dansent, mais aucun son.',
...
assert.equal((await post(input('chat',2,{epoch:perceptionEpoch,message:'Réponds'}))).status,423);assert.equal(calls,chatCalls+1);
...
assert.equal((await post(input('chat',1,{epoch,message:'Tu dors ?'}))).status,423);assert.equal(calls,before);
r=await post(input('interact',2,{epoch}));assert.equal(r.status,200);w=await r.json();assert.equal(w.story.life.sleepTurns[1],2);assert.equal(w.agents[0].intent,'none');assert.equal(calls,before);assert.equal(w.messages.filter(m=>m.speaker==='Lia · rêve').length,1);
// Eating and legacy speaker activation emit explicit cues, cached with the turn.
r=await post(input('care',2,{epoch,intent:'eat'}));assert.equal(r.status,200);w=await r.json();assert.ok(w.visualEvents.some(e=>e.kind==='food'&&e.duration>2000));assert.ok(w.departures.some(d=>d.actor===2&&d.from==='salon'&&d.to==='cuisine'&&/manger|faim|avaler/.test(d.content)));assert.equal(calls,before);
...
console.log('Passed: minimum two zero-API sleep turns, one room-stamped dream narration, blocked sleeper chat, explicit food and cached speaker cues, motivated departure, garden priority over contact/debrief, distinct truthful appearance and shared scene registries.');
}
