...
const {referenceSections}=await import('../.sites-runtime/test-reference.mjs');const {updateAudit}=await import('../.sites-runtime/test-update-audit.mjs');assert.equal(updateAudit.length,25);assert.equal(new Set(updateAudit.map(a=>a.point)).size,25);assert.ok(referenceSections[0].title.includes('Version 171'));assert.ok(referenceSections.some(s=>s.title.startsWith('26')&&s.text.includes('18a')&&s.text.includes('20b')));assert.ok(referenceSections.some(s=>s.text.includes('food=3800 ms')));assert.ok(!referenceSections.some(s=>s.text.includes('2 400 ms')));assert.equal(investigationCounts([],[],true,[],{mirrorVerified:true,ambientVerified:true}).observations,3);assert.ok(stockResult.story.life.foodVerified);console.log('Passed: all 25 requested changes listed, current Admin revision and durations, verified legend/count concordance and first food witness validation.');
...

{
  ...
  const {
    ...
    buildCircleRunSummaryHtml, findRegistriesMissingFromCircle, CIRCLE_EXCLUDED_REGISTRIES,
  ...
  // buildCircleRunSummaryHtml() (2026-09-20, trou trouvé par l'utilisateur : « je n'ai pas eu de
  ...
  const emptySummary = buildCircleRunSummaryHtml([]);
  ...
  const realSummary = buildCircleRunSummaryHtml([
