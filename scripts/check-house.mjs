...
const {referenceSections}=await import('../.sites-runtime/test-reference.mjs');const {updateAudit}=await import('../.sites-runtime/test-update-audit.mjs');assert.equal(updateAudit.length,25);assert.equal(new Set(updateAudit.map(a=>a.point)).size,25);assert.ok(referenceSections[0].title.includes('Version 190'));assert.ok(referenceSections.some(s=>s.title.startsWith('26')&&s.text.includes('18a')&&s.text.includes('20b')));assert.ok(referenceSections.some(s=>s.text.includes('food=3800 ms')));assert.ok(!referenceSections.some(s=>s.text.includes('2 400 ms')));assert.equal(investigationCounts([],[],true,[],{mirrorVerified:true,ambientVerified:true}).observations,3);assert.ok(stockResult.story.life.foodVerified);console.log('Passed: all 25 requested changes listed, current Admin revision and durations, verified legend/count concordance and first food witness validation.');
...
}
{
  ...
  assert.equal(trueOk100.message, '🎖️ FAKE-AGENT-COMPLET obtient son badge — intégration complète vérifiée (blueprint, instanciation, registre, mention CLAUDE.md, présence PRESTATIONS). Couverture de code : OK 100%.', 'the announcement message must be exactly the two separate sentences the user validated verbatim — the badge sentence and the coverage sentence — never merged into one, and never a third sentence added');
  ...
  assert.equal(withVerifiedDate.message, '🎖️ FAKE-AGENT-COMPLET obtient son badge — intégration complète vérifiée (blueprint, instanciation, registre, mention CLAUDE.md, présence PRESTATIONS). Couverture de code : OK 100% (vérifié le 2026-09-21).', '2026-09-21 enrichment: a real, caller-supplied lastVerifiedAt date must be appended to the coverage sentence, never fabricated when omitted (the earlier trueOk100 case above proves the sentence stays exactly as validated when no date is supplied)');
