...

{
  ...
  assert.ok(avecVerdict.fautes>conformite.fautes,'a secondary guardian\'s negative verdict must be RELAYED into the single report, never recalculated and never lost');
...

{
  ...
  assert.deepEqual(parsed[0],{tool:'check-house.mjs',cout:'gratuit',declenchement:'à chaque changement de code',statut:null,description:'régressions de comportement'},'a backtick-quoted tool name must be read with the backticks stripped, and its real cost/trigger columns kept exactly as written; without a Statut column in the source table, statut must be null rather than a guessed value, and the description column must be read by name like the others');
...
}
{
  ...
  assert.equal(complete.description, 'fait des choses', 'the result (2026-09-21, fiabilisation du gabarit de certification) must carry a short description read by name from the "Ce qu\'il détecte/régule" column, never a second hand-maintained text');
  assert.deepEqual(complete.companions, [], 'an agent that appears alone (never alongside another tool) in every real PRESTATIONS entry must report an honest empty companions list, never a fabricated combination');
  const withCombo = checkAgentOnboarding('FAKE-AGENT-COMPLET', { toolsTableMarkdown: fakeTable, prestations: [...fakePrestations, { demande: 'Faire des choses avec un ami', outils: ['FAKE-AGENT-COMPLET', 'AUTRE-OUTIL'], cout: 'gratuit' }], existingPaths: completePaths });
  assert.deepEqual(withCombo.companions, ['AUTRE-OUTIL'], 'once a real PRESTATIONS entry lists this agent alongside another tool, that other tool must be reported as a real companion, derived from the existing catalogue rather than a new hand-maintained field');
  ...
  // 2026-09-22 — cette assertion affirmait "OK 100%" pour un appel qui ne fournit QUE 4 des 6
  // signaux : elle épinglait donc le défaut « Gardien non consulté = Gardien vert » plutôt qu'un
  // comportement voulu (même correction qu'un test épinglant un faux positif, déjà faite deux fois
  // cette session). Le cas garde tout son intérêt sous sa forme honnête : 4 signaux verts sur 6 ne
  // sont pas "OK 100%", ils sont une vérification encore en cours.
  const quatreSurSix = checkAgentOnboarding('FAKE-AGENT-COMPLET', { toolsTableMarkdown: fakeTable, prestations: fakePrestations, existingPaths: completePaths, axaCoveragePct: 100, argusFindingsCount: 0, harmoniaFindingsCount: 0, cleanDirtyOldFlagged: false });
  assert.deepEqual(quatreSurSix.couverture, { tier: 'en cours', label: 'en cours (4/6 Gardiens au vert — CLONE-HUNTER, ALWAYS-NEW-CODE non consulté(s) à ce relevé)' }, '"OK 100%" must never be awarded while two of the six Gardiens were never consulted at all: an undefined signal is an absence of measurement, never a green one — the same "absence = green" confusion already fixed for mention-vs-launch and for missing keys. The 3-tier scale stays untouched (user-calibrated, task #224): an incomplete verification IS "en cours", only the label gets precise about which Gardiens have not answered');
  ...
  const cinqSurSix = checkAgentOnboarding('FAKE-AGENT-COMPLET', { toolsTableMarkdown: fakeTable, prestations: fakePrestations, existingPaths: completePaths, axaCoveragePct: 100, argusFindingsCount: 0, harmoniaFindingsCount: 0, cleanDirtyOldFlagged: false, cloneHunterFindingsCount: 0 });
  assert.deepEqual(cinqSurSix.couverture, { tier: 'en cours', label: 'en cours (5/6 Gardiens au vert — ALWAYS-NEW-CODE non consulté(s) à ce relevé)' }, 'same correction one notch up: five explicitly clean Gardiens and one never consulted is still not "OK 100%" — and the label must name exactly which one is missing, so the reader can tell "nobody looked" from "everybody looked and it is clean"');

  // 6e Gardien sacré (2026-09-21, ALWAYS-NEW-CODE — couche légère seulement) : même défaut
  // structurel reproduit puis corrigé une troisième fois, exactement le même patron que le 5e
  // Gardien (CLONE-HUNTER) ci-dessus.
  const missingSixthMember = checkAgentOnboarding('FAKE-AGENT-COMPLET', { toolsTableMarkdown: fakeTable, prestations: fakePrestations, existingPaths: completePaths, axaCoveragePct: 100, argusFindingsCount: 0, harmoniaFindingsCount: 0, cleanDirtyOldFlagged: false, cloneHunterFindingsCount: 0, alwaysNewCodeFlagged: true });
  assert.equal(missingSixthMember.couverture.tier, 'partiel', 'a genuine ALWAYS-NEW-CODE light-layer signal must keep the tier at "partiel" even when the other five Gardiens are clean — "OK 100%" is not just the original 5 anymore');
  assert.equal(missingSixthMember.couverture.label, 'partiel (KO ALWAYS-NEW-CODE)', 'an ALWAYS-NEW-CODE signal must be named explicitly as "KO ALWAYS-NEW-CODE", the same binary format as the other Gardiens');
  const trueOk100WithSixth = checkAgentOnboarding('FAKE-AGENT-COMPLET', { toolsTableMarkdown: fakeTable, prestations: fakePrestations, existingPaths: completePaths, axaCoveragePct: 100, argusFindingsCount: 0, harmoniaFindingsCount: 0, cleanDirtyOldFlagged: false, cloneHunterFindingsCount: 0, alwaysNewCodeFlagged: false });
  assert.deepEqual(trueOk100WithSixth.couverture, { tier: 'OK 100%', label: 'OK 100%' }, '"OK 100%" is still reachable once all SIX Gardiens are explicitly reported clean, alwaysNewCodeFlagged included');
  assert.equal(trueOk100WithSixth.message, '🎖️ FAKE-AGENT-COMPLET obtient son badge — toutes les validations réunies : table maîtresse (docs/regles-de-travail.md §7ter), ajouté au catalogue du coordinateur (le-catalogue-du-coordinateur), instanciation (docs/referentiel/fake-agent-complet.md), registre (docs/fake-agent-complet/), blueprint (docs/fake-agent-complet-blueprint.md). Couverture de code : OK 100%.', '2026-09-22 rewrite (demande explicite : « je veux toutes les validations ») : la liste n\'est plus un texte figé mais reflète exactement ce qui a été vérifié pour ce cas précis (claudeMdText/suiviText/hasDocReportDecision/reciprocalWiring omis ici, donc absents de la liste — jamais fabriqués)');
  const withVerifiedDate = checkAgentOnboarding('FAKE-AGENT-COMPLET', { toolsTableMarkdown: fakeTable, prestations: fakePrestations, existingPaths: completePaths, axaCoveragePct: 100, argusFindingsCount: 0, harmoniaFindingsCount: 0, cleanDirtyOldFlagged: false, cloneHunterFindingsCount: 0, alwaysNewCodeFlagged: false, lastVerifiedAt: '2026-09-21' });
  assert.equal(withVerifiedDate.message, '🎖️ FAKE-AGENT-COMPLET obtient son badge — toutes les validations réunies : table maîtresse (docs/regles-de-travail.md §7ter), ajouté au catalogue du coordinateur (le-catalogue-du-coordinateur), instanciation (docs/referentiel/fake-agent-complet.md), registre (docs/fake-agent-complet/), blueprint (docs/fake-agent-complet-blueprint.md). Couverture de code : OK 100% (vérifié le 2026-09-21).', '2026-09-21 enrichment: a real, caller-supplied lastVerifiedAt date must be appended to the coverage sentence, never fabricated when omitted (the trueOk100WithSixth case above proves the sentence stays exactly as validated when no date is supplied)');

  // Relevé partagé des 6 signaux (2026-09-22) — l'autre moitié du même défaut : le crochet
  ...
  const viaSnapshot = checkAgentOnboarding('FAKE-AGENT-COMPLET', { toolsTableMarkdown: fakeTable, prestations: fakePrestations, existingPaths: completePaths, ...badgeSignalsAsContext({ date: '2026-09-22', argusFindingsCount: 0, harmoniaFindingsCount: 0, cleanDirtyOldFlagged: false, cloneHunterFindingsCount: 0, alwaysNewCodeFlagged: false, coverageBySlug: { 'fake-agent-complet': 82 } }) });
  assert.equal(viaSnapshot.couverture.label, 'partiel (KO AXA-CHECK 82%)', 'the real discrepancy this fixes: a caller with no per-tool percentage of its own must now read the shared snapshot and report the measured 82%, instead of claiming the tool was never scanned');
  const perOutilGagne = checkAgentOnboarding('FAKE-AGENT-COMPLET', { toolsTableMarkdown: fakeTable, prestations: fakePrestations, existingPaths: completePaths, axaCoveragePct: 100, axaCoverageBySlug: { 'fake-agent-complet': 82 }, argusFindingsCount: 0, harmoniaFindingsCount: 0, cleanDirtyOldFlagged: false, cloneHunterFindingsCount: 0, alwaysNewCodeFlagged: false });
  assert.equal(perOutilGagne.couverture.tier, 'OK 100%', 'a caller that measured THIS tool itself (axaCoveragePct) always wins over the shared snapshot, which may be a commit old — the snapshot is a fallback, never an override');
  const snapshotAllerRetour = path.join(os.tmpdir(), `badge-signals-${Date.now()}.json`);
  saveSignals({ commit: 'abc123', date: '2026-09-22', argusFindingsCount: 4 }, snapshotAllerRetour);
  assert.deepEqual(loadSignals(snapshotAllerRetour), { commit: 'abc123', date: '2026-09-22', argusFindingsCount: 4 }, 'the snapshot must survive a real write/read round-trip on disk, commit included so a reader knows which state of the repo it describes');
  assert.equal(loadSignals(path.join(os.tmpdir(), `badge-signals-absent-${Date.now()}.json`)), null, 'a missing snapshot file must return an honest null (→ empty context → "jamais consulté"), never crash the badge of every tool at once');
...

{
  ...
  assert.ok(block.includes('Description : non renseignée'), 'the fiabilisé gabarit (2026-09-21) must always show a Description line — an honest explicit placeholder when the source data lacks one, never a silently omitted line');
  assert.ok(block.includes('Combine typiquement avec : aucune combinaison connue'), 'the gabarit must always show a "Combine typiquement avec" line — an honest explicit absence when no real PRESTATIONS entry lists this tool alongside another, never a silently omitted line');
  const fakeCertifiedWithData = { ...fakeCertified, description: 'traque les doublons', companions: ['AXA-CHECK', 'ARGUS'] };
  const blockWithData = formatBadgeCeremonyAnnouncement(fakeCertifiedWithData);
  assert.ok(blockWithData.includes('Description : traque les doublons'), 'when checkAgentOnboarding() supplies a real description, the gabarit must show it verbatim, never a paraphrase');
  assert.ok(blockWithData.includes('Combine typiquement avec : AXA-CHECK, ARGUS'), 'when real companions are supplied, the gabarit must list them, same format every time — the exact 3 things the user asked for: a short description, whether everything is plugged in, and which tools it typically combines with');
...

{
  ...
  // Ce test affirmait que regles-de-travail.md doit rendre un gain > 0. Il épinglait en réalité un
  // FAUX POSITIF : la stratégie « déjà mécanisé » y proposait 6 134 tk sur 32 passages qui étaient
  // tous des règles. Ce document EST le manuel des procédures du projet (18 fichiers le désignent
  // comme tel) — lui reprocher d'en décrire, c'est lui reprocher d'exister. L'intention du test
  // reste bonne (prouver que l'outil marche au-delà de CLAUDE.md) ; elle se vérifie désormais sur
...

{
  ...
  assert.equal(report.length, 27, 'buildCircleReport() must return exactly one entry per CIRCLE_ITEMS item, in the same order, never dropping or reordering one — 27 since god-of-all-process-conformite joined CIRCLE_ITEMS on 2026-09-22, after organigramme-signal, check-tasks-report and hyper-scan-checkpoint-light the same day');
  ...
  // chantier-preliminaire-signal (2026-09-21, extension explicite de checkChantierFileFreshness()
  // demandée par l'utilisateur) — vérifié live contre le vrai docs/suivi/ et le vrai registre
...

{
  ...
  assert.deepEqual(mismatches.map((m) => m.slug), ['unwired-html-tool'], 'the top-level mismatches list must surface exactly the real HTML-wiring gap, ready for a human/agent to read — Doc-Report itself never fixes it');
    // « JAMAIS SOLLICITÉ » RECOUVRE DEUX CHOSES (2026-09-22, trouvé en lançant Doc-Report pendant la
  // Ronde finale) : un outil réellement jamais lancé, et un outil dont le script n'enregistre rien,
  // que le compteur ne pourrait pas voir même lancé dix fois par jour. Les confondre, c'est lire une
  // absence de mesure comme une mesure.
  const {scriptRecordsItsUsage}=await import('../scripts/doc-report.mjs');
  assert.equal(scriptRecordsItsUsage('scripts/x.mjs',()=>'  recordCliUsage("x");'),true,'a script that genuinely records its own run must be recognised, read from the real file rather than a hand-kept list');
  assert.equal(scriptRecordsItsUsage('scripts/x.mjs',()=>'console.log("rien")'),false,'a script with no recording point must be reported as such, since its zero says nothing about how often it actually ran');
  assert.equal(scriptRecordsItsUsage('scripts/absent.mjs',()=>{throw new Error('ENOENT');}),false,'an unreadable script must answer honestly rather than crash the whole index');
  assert.equal(scriptRecordsItsUsage(undefined),false,'a registry entry carrying no script path must never be guessed either way');
...
{
  // ORGANIGRAMME RECONSTRUIT PAR CASSANDRA (tâches #171/#172/#179, 2026-09-22 — calibrage explicite :
  // « CASSANDRA le reconstruit à chaque fois »). Trois tâches de suivi ouvertes depuis des jours
  // demandaient qu'elle tienne réellement la liste de l'équipe ; la tenue manuelle de
  // docs/referentiel/organisation-agence.md ne tenait pas — preuve directe, TROIS membres certifiés
  // n'appartenaient à aucune suite depuis leur certification sans que personne ne l'ait vu.
  const { buildOrganigramme, findMembersWithoutCategory, renderOrganigrammeReport, ORG_RANKS } = await import('../scripts/cassandra-rh.mjs');
  const vraieTable = fs.readFileSync('docs/regles-de-travail.md', 'utf8');
  ...
  const doublons = [...org.socle, ...org.cadres, ...org.gardiens, ...Object.values(org.suites).flat(), ...org.emetteurs];
  const { toolIdentitySlug } = await import('../scripts/le-coordinateur.mjs');
  const slugs = doublons.map(toolIdentitySlug);
  assert.equal(new Set(slugs).size, slugs.length, 'no tool may appear at two ranks at once — the master table writes "Doc-Report" and "check-spirit.mjs" while the disk writes "doc-report.mjs" and "check-spirit", and both really did show up twice on the first real render');
  assert.equal(toolIdentitySlug('check-spirit.mjs'), toolIdentitySlug('check-spirit'), 'the identity slug must recognise the same tool named with and without its file extension — the exact collision that placed check-spirit at two ranks');
  assert.equal(toolIdentitySlug('Smart Breaker (check-gemini-quota.mjs + …)'), 'smart-breaker', 'a name carrying a parenthetical precision must resolve to its primary identity, the same primaryName rule used everywhere else — never a third naming convention');
  assert.notEqual(toolIdentitySlug('check-house.mjs'), 'check-house-mjs', 'the identity slug must NOT be slugifyAgentName(): that one builds a documentation PATH and must never change, this one answers a different question');

  // Fabriqué de bout en bout, pour ne dépendre d'aucun état réel du dépôt.
  const fausseTable = '| Outil | Coût | Déclenchement | Statut | Description |\n|---|---|---|---|---|\n| ALPHA | gratuit | à la demande | Agent | fait des choses |\n| BETA | gratuit | à la demande | Agent | fait autre chose |\n| socle.mjs | gratuit | toujours | Infrastructure | le sol |\n| GAMMA | gratuit | à la demande | Utilitaire nommé | écrit un rapport |\n';
  const faux = buildOrganigramme({ toolsTableMarkdown: fausseTable, categories: { alpha: 'Agent Cadre', beta: 'Membre — Suite Test' }, gardienDomains: {}, fileWriterNatures: {}, registries: [] });
  assert.deepEqual(faux.cadres, ['ALPHA'], 'a tool categorised Agent Cadre must land in the Cadre rank, read from the real categories map rather than a name pattern');
  assert.deepEqual(faux.suites, { 'Membre — Suite Test': ['BETA'] }, 'suites must be built from the real categories found, never from a fixed list of suite names that would go stale the day a suite is added');
  assert.deepEqual(faux.emetteurs, ['GAMMA'], 'a master-table tool that is neither certifiable nor infrastructure is a report emitter — the exact scope the user chose for the future 🥈 rank, derived rather than listed');
  assert.deepEqual(findMembersWithoutCategory(fausseTable, { alpha: 'Agent Cadre' }), ['BETA'], 'a certified member missing from the categories map must be named, never silently dropped from every suite');

  const texte = renderOrganigrammeReport(org, { dateLabel: '2026-09-22' });
  assert.ok(texte.includes('CASSANDRA-RH — organigramme'), 'the org chart must render through the shared report gabarit, never its own private layout');
...

{
  ...
  assert.deepEqual(badgeSummary, { total: 2, certified: 1, notCertified: [{ agentName: 'B', gaps: ['instanciation manquante'] }], changements: null }, 'the oversight summary must split certified/notCertified honestly from whatever real checkAgentOnboarding() results it is handed, never recomputing coverage itself — and with no previous states supplied it must report an honest null, never an empty list that would read as "nothing moved"');

  // CE QUI A BOUGÉ (2026-09-22) : CASSANDRA supervisait les badges sur la seule PHOTO du jour
  // (certifié / pas certifié) et ne pouvait donc jamais dire ce qui s'était dégradé depuis la
  // dernière fois — alors que la cérémonie, elle, annonce désormais ces changements de son côté.
  // Elle lit les états déjà écrits par badgeState()/announceBadgeChange(), jamais un second calcul.
  const avecEtats = (tier, nbGaps) => badgeOversightSummary(
    [{ agentName: 'A', slug: 'a', complet: true, badge: '🎖️ Membre certifié', gaps: Array(nbGaps).fill('x'), couverture: { tier } }],
    { a: { badge: '🎖️ Membre certifié', tier: 'OK 100%', gaps: 0 } });
  assert.deepEqual(avecEtats('OK 100%', 0).changements, [], 'states supplied and nothing moved must give an empty list — distinct from null, which means nobody ever looked');
  assert.equal(avecEtats('partiel', 0).changements[0].degradation, true, 'losing the "OK 100%" tier is a degradation even when the number of missing validations has not budged — otherwise half the bad news would read as neutral');
  assert.equal(avecEtats('OK 100%', 1).changements[0].degradation, true, 'gaining a missing validation is a degradation too');
  assert.deepEqual(badgeOversightSummary([{ agentName: 'Z', slug: 'z', complet: true, badge: 'b', gaps: [], couverture: { tier: 'OK 100%' } }], { a: {} }).changements, [], 'an agent with no previously recorded state must be silently skipped, never announced as having changed — the same absence-vs-measurement discipline as the ceremony itself');
  ...
  // Nouveaux visages (Phase 1, 2026-09-21) — detectNewArrivals()/narrateNewArrivals() restent purs,
  // jamais de disque touché ici (loadKnownMembers()/recordKnownMembers() sont testées séparément
  // juste après, avec un vrai fichier temporaire).
  const arrivalRoster = [
    { tool: 'Outil Connu', primaryName: 'Outil Connu', slug: 'outil-connu' },
    { tool: 'Outil Neuf Complet', primaryName: 'Outil Neuf Complet', slug: 'outil-neuf-complet' },
    { tool: 'Outil Neuf Incomplet', primaryName: 'Outil Neuf Incomplet', slug: 'outil-neuf-incomplet' },
  ];
  const newArrivals = detectNewArrivals(arrivalRoster, ['outil-connu']);
  assert.deepEqual(newArrivals.map((m) => m.slug), ['outil-neuf-complet', 'outil-neuf-incomplet'], 'a member whose slug is already in the known list must never be reported as a new arrival, and every genuinely unknown slug must be, in roster order');
  assert.deepEqual(detectNewArrivals(arrivalRoster, arrivalRoster.map((m) => m.slug)), [], 'a roster with every slug already known must report zero new arrivals, never a false positive');

  const arrivalBadgeResults = [
    { agentName: 'Outil Neuf Complet', complet: true, badge: '🎖️ Membre certifié', gaps: [] },
    { agentName: 'Outil Neuf Incomplet', complet: false, badge: '⚠️ Pas encore certifié', gaps: ['blueprint manquant'] },
  ];
  const narration = narrateNewArrivals(newArrivals, arrivalBadgeResults);
  assert.equal(narration.length, 2, 'one narration line per new arrival, never more or fewer');
  assert.ok(narration[0].includes('Outil Neuf Complet') && narration[0].includes('déjà complète'), 'a new arrival that is already complete must be narrated as such, badge included, never left ambiguous');
  assert.ok(narration[1].includes('Outil Neuf Incomplet') && narration[1].includes('blueprint manquant'), 'a new arrival still incomplete must be narrated with its exact real gaps, never a vague "pas encore prêt"');

  const blocksWithArrivals = buildCassandraReportBlocks({ teamSize, badgeSummary, kpiTrend: trend, reconsider: [], newArrivalsNarration: narration });
  assert.ok(blocksWithArrivals[0].type === 'heading' && blocksWithArrivals[0].text === 'Nouveaux visages à l\'Agence Codex', 'a report with real new arrivals must lead with their section, first thing visible, matching the explicit "message ici dans la conversation" request');
  assert.ok(!buildCassandraReportBlocks({ teamSize, badgeSummary, kpiTrend: trend, reconsider: [] }).some((b) => b.text?.includes('Nouveaux visages')), 'a report with zero new arrivals (the default) must never fabricate an empty section — silence is the honest signal here, same discipline as "Recrutement en cours"');

  // loadKnownMembers()/recordKnownMembers() — vrai fichier temporaire, jamais le vrai journal du
  // dépôt touché par un test (même discipline que les autres journaux locaux de ce projet).
  const { mkdtempSync: mkTmpDirArrivals, rmSync: rmTmpArrivals } = await import('node:fs');
  const { tmpdir: osTmpDirArrivals } = await import('node:os');
  const tmpArrivalsDir = mkTmpDirArrivals(path.join(osTmpDirArrivals(), 'cassandra-rh-known-members-'));
  const tmpArrivalsPath = path.join(tmpArrivalsDir, 'known.json');
  try {
    assert.deepEqual(loadKnownMembers(tmpArrivalsPath), { slugs: [] }, 'a journal never yet written must report an honest empty list, never crash');
    recordKnownMembers(['a', 'b', 'a'], tmpArrivalsPath);
    assert.deepEqual(loadKnownMembers(tmpArrivalsPath).slugs.sort(), ['a', 'b'], 'the journal must persist a deduplicated set of slugs, never a repeated entry that would inflate a future diff for no reason');
  } finally {
    rmTmpArrivals(tmpArrivalsDir, { recursive: true, force: true });
  }

  console.log('Passed: CASSANDRA-RH (2026-09-21, noyau fiabilisé + trous d\'équipe) — the fixed persona text stays verbatim, parseKpiHistoryCsv()/latestKpiTrend() read kpi-historique.csv honestly (an absent cell stays undefined, a family missing a measurement is skipped rather than fabricated), teamRoster() correctly resolves a tool name carrying a parenthetical precision to its real AGENT_CATEGORIES entry — the exact real bug found and fixed tonight, verified live against the actual master table with zero uncategorized Agent remaining — teamSizeSnapshot() gives an honest plain headcount never a judged verdict, toolsToReconsider() combines tool-usage.mjs/clean-dirty-old.mjs signals without a third invented calculation, the recruitment skeleton enforces an explicit decision at every stage and never reopens a closed file, badgeOversightSummary()/computeBadgeResults() supervise checkAgentOnboarding()\'s real results without ever recalculating coverage themselves (and, verified live, resolve a parenthetical Agent name correctly, the same fix already proven for checkAllAgentBadges()), computeCoverageGaps() reads real AXA-CHECK coverage to flag a fragile or never-scanned team member — "trous d\'équipe" resolved explicitly as team/coverage gaps, never the separate document-structure role reserved for LE-GRAND-ARCHITECTE — never relaunched in the light signal (coverageGaps stays null there, [] never fabricated), only in the explicit full report, and the light signal/full HTML report otherwise read the exact same underlying data — and (2026-09-21, Phase 1) detectNewArrivals()/narrateNewArrivals() give CASSANDRA her own "nouveaux visages" narration, distinct from the mechanical badge ceremony (presence vs completeness), persisted one-time via loadKnownMembers()/recordKnownMembers() with a real deduplicated journal, leading the full report exactly once per real new slug and never fabricating the section when there is nothing to announce.');
}
