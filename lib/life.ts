...
themeFrequency?:Record<string,number>;tournureFrequency?:Record<string,number>;
// MÉMOIRE DES MOTIFS DE DÉPLACEMENT (2026-09-23, tâche #592) — le seul canal affiché à l'écran
// qui n'avait AUCUNE protection contre la répétition. Les répliques ont wordFrequency, les thèmes
// ont themeFrequency, les phrases ont le registre d'empreintes ; le motif de déplacement, lui,
// était réécrit à neuf chaque tour par un modèle qui ne pouvait pas savoir ce qu'il avait déjà
// dit. Résultat mesuré sur full_sim19 : « souffler » quatre fois dans quatre motifs successifs,
// le mot que le corollaire de l'Article 17 cite précisément comme son cas d'école. Ce n'est donc
// pas une liste de mots de plus, c'est la mémoire qui manquait à ce canal. Par personnage, pour
// que les deux voix restent étanches (Article 11).
moveReasons?:Record<string,string[]>;recapCount?:number;visualIntro?:number;ambientSeen?:boolean;personalFollowup?:number;observerNamed?:boolean;causeByActor?:Record<string,string>;remoteFound?:boolean;exitSearched?:boolean;exitPhase?:number;exitActive?:boolean;discussedObjects?:string[];proposalMade?:boolean;proposalHistoryChecked?:boolean;personalAsked?:boolean;personalRound?:number;personalBoosted?:boolean;personalConcluded?:boolean;visited:Room[];attachment:Record<string,number>;credit:Record<string,number>;debrief?:{topic:string;remaining:number};contact?:{room:Room;remaining:number};dispute?:{topic:string;remaining:number};studyTurns:number;contacts:number[];
