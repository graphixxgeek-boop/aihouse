...
export async function readWorld(db: D1Database) {
    ...
    const story = (() => { try { const data = JSON.parse(scenario?.content ?? "null"); return data ? {observer:data.observer,humanUnlocked:data.finalCalled===true && data.evidence.length>=5, session:data.seed, evidence:data.evidence as string[], dreams:(data.dreams??[]) as {actor:1|2;round:number;content:string}[], observations:(data.observations??[]) as string[], life:readLife(data.life,data.round),revealed:data.evidence.length >= 5} : null; } catch { return null; } })();
