 ...
 * rotation, jamais toujours dans le même ordre), les clés en cooldown en dernier recours. Fait
 * aussi avancer la rotation pour le prochain appel — à n'appeler qu'une fois par tentative réelle.
 */
export function orderKeys(rawKeys: readonly string[]): number[] {
    const now = Date.now();
    const n = rawKeys.length;
    const order = rawKeys.map((_, i) => i).sort((a, b) => {
        const coolA = (cooldownUntil.get(rawKeys[a]) ?? 0) > now ? 1 : 0;
        const coolB = (cooldownUntil.get(rawKeys[b]) ?? 0) > now ? 1 : 0;
...
export function recordKeyStatus(rawKey: string, status: number): void {
    ...
    if (status === 429) { cooldownUntil.set(rawKey, Date.now() + QUOTA_COOLDOWN_MS); return; }
    if (status === 503) { cooldownUntil.set(rawKey, Date.now() + TRANSIENT_COOLDOWN_MS); return; }
...
export function __resetGeminiKeyRotationForTests(): void { rotation = 0; cooldownUntil.clear(); }
